...
char* readUserFile ()
{
	FILE *fp;
	char *namepass;
	namepass = (char*)malloc(MAXDATASIZE);
	
	fp = fopen("patient1.txt", "r");
	if(fp == NULL)
	{
		perror("Error when opening patient1.txt");
		exit(-1);
	}
	if(fgets(namepass, MAXDATASIZE, fp) == NULL)
	{
...
*/
string patientInfo() {
	ifstream fin("patient1.txt");
	string namepass;
	if(fin == NULL) {
		perror("Error opening patient1.txt");
		exit(-1);
	}
	if(getline(fin, namepass) == NULL) {
		perror("Error getting Patient1 Information");
		fin.close();
		exit(-1);
	}
	fin.close();
...
string insuranceInfo() {
	ifstream fin("patient1insurance.txt");
	string namepass;
	if(fin == NULL) {
		perror("Error opening patient1.txt");
		exit(-1);
...
*/
	...
	cout<<"Phase 1: Patient 1 has TCP port number "<<sin.sin_port<<" and IP address "<<myIP<<"."<<endl;
...
//------------------ReadUserFile and send to server-------------------------------
    ...
    string patient1 = patientInfo();
    vector<string> patient1UP = split(patient1);
	patient1 = "authenticate " + patient1;
	//cout<<patient1<<endl;

    if (send(sockfd, patient1.c_str(), patient1.length() + 1, 0) == -1) 
	{
    	perror("Send Username and Password Failure.");
	}
	cout<<"Phase 1: Authentication request from Patient 1 with username "<<patient1UP[0]<<" and password "<<patient1UP[1]<<" has been sent to the Health Center Server."<<endl;
...
//-------------------------get authentication response-------------------------------	
	...
	if(auth == "success") {
		cout<<"Phase 1: Patient 1 authentication result: success."<<endl;
		cout<<"Phase 1: End of Phase 1 for Patient1"<<endl;
	}
	else {
		cout<<"Phase 1: Patient 1 authentication result: failure."<<endl;
		exit(1);
	}
...
//--------------receive available reservations----------
	...
	cout<<"Phase 2: The following appointments are available for Patient 1:"<<endl;
...
//-------------------receive reservation confirmation---------------------------
	...
	if(confirm_str == "notavailable") {
		...
		cout<<"Phase 2: The requested appointment is available and reserved to Patient1. The assigned doctor port number is "<<confirm_VecStr[1]<<"."<<endl;
		close(sockfd);
	}
//--------------------------------------Phase 3-----------------------------------------------------
//----------------create UDP-------------------------------
...
//The getsockname retrieves the local address associated with the socket. The UDT socket must be bound explicitly (via bind) or implicitly (via connect), otherwise this method will fail because there is no meaningful address bound to the socket.
	...
	cout<<"Phase 3: Patient 1 has a dynamic UDP port number "<<sin_udp.sin_port<<" and IP address "<<myIP<<"."<<endl;
 ...
 //---------------------------send insurance to doctor--------------------------------
    ...
    cout<<"Phase 3: The cost estimation request from Patient 1 with insurance plan "<<insurance<<" has been sent to the doctor with port number "<<confirm_VecStr[1]<<" and IP address "<<myIP<<"."<<endl;
 ...
 //----------------------------------get price----------------------------------------   
    ...
    cout<<"Phase 3: Patient 1 receives "<<buf_price<<"$ estimation cost from doctor with port number "<<confirm_VecStr[1]<<" and name "<<insurance<<"."<<endl;
    cout<<"Phase 3: End of Phase 3 for Patient 1."<<endl;
